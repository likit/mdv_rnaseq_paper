...
allgenes<-read.table('protocol/Homo_sapiens.GRCh37.74.pep.all.txt', stringsAsFactors=F)
...
get_genes_kegg = function(cat, data, prefix)
{
    ...
    mm = m[!is.na(m)]
    d = data.frame(cat, data[mm,]$ENTREZID, data[mm,]$geneID, rownames(data[mm,]), data[mm,]$E.value)
...
# write.table(KEGG_SIG, 'line7_pre_post_DGE_GOSeq_KEGG_summary.txt', sep='\t', row.names=F, quote=F)
