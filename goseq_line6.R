...
degenes.table.uniq<-degenes.table[
                      !duplicated(degenes.table$geneID),]
...
gene.vector<-as.integer(
            allgenes$ensembl_gene_id%in%degenes.table.uniq$geneID)
...
pathway = stack(mget(KEGG[KEGG$padjust<0.5,]$category,
                     KEGGPATHID2NAME))
...
get_genes_kegg = function(cat, data, prefix)
{
    ...
    mm = m[!is.na(m)]
    d = data.frame(cat, data[mm, ]$id, data[mm,]$geneID,
